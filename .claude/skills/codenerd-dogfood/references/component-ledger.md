...
- **Open:** **F-REVIEW-TIMEOUT-1** — `nerd review` of the ~700-line auditor could not emit a verdict at either 12 minutes (10 iterations) or the documented 25-minute default (17 iterations). Both runs stayed responsive and then died in tool-result follow-up on context deadline. Do not treat either as semantic approval; investigate deadline-aware forced finalization next.
