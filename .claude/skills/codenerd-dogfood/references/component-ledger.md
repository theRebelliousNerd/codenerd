...
- **Verification boundary:** exact changed-path race, focused core/store, vet, real-kernel schema, and core strict+verify gates pass. The default full-package fan-out timed out four heavy packages that pass separately, and the broad core race run exposed an intermittent ANTLR parser race in `TestShardManager_ConcurrentToolQueries`; neither residual is relabeled green.
