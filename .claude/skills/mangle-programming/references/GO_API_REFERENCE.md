...
**Version**: `codeberg.org/TauCeti/mangle-go v0.5.1-0.20260413190942-4dcaa582c6d3` (April 2026)
**Repository**: <https://codeberg.org/TauCeti/mangle-go>
**Upstream base**: `github.com/google/mangle v0.4.0` (Nov 2024)

Comprehensive reference for integrating the TauCeti Mangle fork into Go applications. Fork-only packages and engine options are flagged in-line and detailed in [960-FORK_FEATURES_v0.5.1](960-FORK_FEATURES_v0.5.1.md).
...
|---------|-------------|---------|
| `ast` | `codeberg.org/TauCeti/mangle-go/ast` | AST types (Atom, Constant, Variable, intervals) |
| `parse` | `codeberg.org/TauCeti/mangle-go/parse` | Parsing Mangle source code |
| `analysis` | `codeberg.org/TauCeti/mangle-go/analysis` | Program analysis, stratification, temporal validation |
| `engine` | `codeberg.org/TauCeti/mangle-go/engine` | Semi-naive evaluation, recorder hooks, gas limits |
| `factstore` | `codeberg.org/TauCeti/mangle-go/factstore` | Fact stores (in-memory, simplecolumn raw/gzip/**zstd**, interval-tree) |
| `builtin` | `codeberg.org/TauCeti/mangle-go/builtin` | Built-in predicates and functions (incl. `:interval:`) |
| `symbols` | `codeberg.org/TauCeti/mangle-go/symbols` | Built-in symbols, type-expression constructors (incl. `NewUnionType`) |
| `unionfind` | `codeberg.org/TauCeti/mangle-go/unionfind` | Unification implementation (`Clone()` available in fork) |
| `json2struct` | `codeberg.org/TauCeti/mangle-go/json2struct` | JSON to Mangle struct conversion |
| `proto2struct` | `codeberg.org/TauCeti/mangle-go/proto2struct` | Protobuf to Mangle struct conversion |
| `provenance` | `codeberg.org/TauCeti/mangle-go/provenance` | **Fork-only.** `MemoryRecorder`, `BuildFromRecording`, `Explain`, `ProofNode` (`KindLetRow`, `KindDoAggregate`). |
| `cmd/mgwhy` | `codeberg.org/TauCeti/mangle-go/cmd/mgwhy` | **Fork-only CLI.** Why-derivations. |
| `cmd/scinfo` | `codeberg.org/TauCeti/mangle-go/cmd/scinfo` | **Fork-only CLI.** Inspect simplecolumn bundles. |
...
```go
import "codeberg.org/TauCeti/mangle-go/json2struct"
...
## Fork-Only API Surface (v0.5.1)

The packages below exist only in the TauCeti fork. Detailed walk-throughs live in [960-FORK_FEATURES_v0.5.1](960-FORK_FEATURES_v0.5.1.md); this section is the API summary.
...
// Recording
rec := provenance.NewMemoryRecorder()       // implements engine.DerivationRecorder
events := rec.Events()                       // []*Event
forFact := rec.EventsFor(atom)               // []*Event matching an output atom

// Proof construction
proofs, err := provenance.BuildFromRecording(rec, store, goalAtom, provenance.Options{
    MaxProofs: 3,
    MaxDepth:  32,
})

// Or, post-hoc (no recorder; plain Datalog only, no let/do):
proofs, err := provenance.Explain(programInfo, store, goalAtom, provenance.Options{})

// Output
provenance.Print(os.Stdout, proofs)          // indented tree
outStore := factstore.NewSimpleInMemoryStore()
provenance.EmitFacts(proofs, &outStore)      // proves/uses_rule/premise/edb_leaf/binding/rule_source
```

`provenance.ProofNode.Kind` adds two kinds beyond `KindEDB`, `KindDerived`, `KindAbsence`:
`KindLetRow` (one row from a let-transform) and `KindDoAggregate` (one group from a do-transform).

### `engine` — new options

```go
import "codeberg.org/TauCeti/mangle-go/engine"

engine.WithDerivationRecorder(rec)            // capture every rule/let/do firing
engine.WithCreatedFactLimit(1_000_000)        // gas limit; aborts when reached
engine.WithExternalPredicates(externalFn)     // supply facts on demand for opaque predicates
engine.WithTemporalStore(temporalStore)       // interval-tree-indexed temporal store
engine.WithEvaluationTime(time.Now())         // pin 'now' for temporal rules
engine.WithNowMarker()                        // mark 'now'-dependent rules for invalidation
engine.WithDeterministicOrder()               // reproducible visit order — use in production
```

### `factstore` — simplecolumn additions

```go
import "codeberg.org/TauCeti/mangle-go/factstore"

store, err := factstore.NewSimpleColumnStoreFromBytes(raw)        // raw
store, err := factstore.NewSimpleColumnStoreFromGzipBytes(gz)     // gzip
store, err := factstore.NewSimpleColumnStoreFromZstdBytes(zst)    // NEW: zstd

count := store.EstimateFactCount()
predCount := store.FactCount(predicateSym)
preds := store.ListPredicates()

// Temporal
ts := factstore.NewIntervalTreeStore()   // pass to engine.WithTemporalStore
```

### `symbols` — type construction

```go
import "codeberg.org/TauCeti/mangle-go/symbols"

ut := symbols.NewUnionType(typeA, typeB)     // programmatic union types
// Tagged unions: use the parser to read .TaggedUnion<...> source rather than constructing by hand.
```

### `unionfind` — Clone

```go
clone := uf.Clone()   // safe branching for speculative evaluation
```
...
|--------|---------|---------|
| `mgwhy` | `go install codeberg.org/TauCeti/mangle-go/cmd/mgwhy@latest` | Why-provenance — see [960](960-FORK_FEATURES_v0.5.1.md#14-the-mgwhy-cli) |
| `scinfo` | `go install codeberg.org/TauCeti/mangle-go/cmd/scinfo@latest` | Inspect simplecolumn fact bundles |

---

## Resources

- **Codeberg (canonical)**: <https://codeberg.org/TauCeti/mangle-go>
- **Go Packages**: <https://pkg.go.dev/codeberg.org/TauCeti/mangle-go>
- **Documentation**: <https://mangle.readthedocs.io>
- **Upstream archive**: <https://github.com/google/mangle> (v0.4.0 base; not the active source)
