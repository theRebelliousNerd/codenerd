...
**Purpose**: Reference for the built-in functions and predicates, with modes and examples.

**Source of truth**: the pinned engine's `symbols/symbols.go` and `builtin/builtin.go`
(`$GOMODCACHE/codeberg.org/!tau!ceti/mangle-go@v0.5.1-0.20260413190942-4dcaa582c6d3/`). This file
was first written from a TypeScript LSP port; on 2026-09-19 every function it documents as existing
was checked against the engine's symbol table and found there. The complete list with arities is
in [150-AI_FAILURE_MODES](150-AI_FAILURE_MODES.md), "Valid Built-in Functions".
...
Reducer functions are used as `let Var = fn:<reducer>(...)` after `|> do fn:group_by(...)` in a transform pipeline.
...
**Examples**:
```mangle
Decl person(ID, Data) bound [/number, fn:Struct(/name, /string)].
Decl person_name(ID, Name) bound [/number, /string].
Decl numbers(ID, L) bound [/name, fn:List(/number)].
Decl suffix(ID, L) bound [/name, fn:List(/number)].
Decl element(ID, Remaining, X) bound [/name, /number, /number].
Decl list_sum(ID, Total) bound [/name, /number].

# Extract struct fields (the struct must be bound by an earlier premise)
person_name(ID, Name) :-
    person(ID, Data),
    :match_field(Data, /name, Name).

# Walk a list bottom-up: generate its suffixes from a relation that binds it
# (a rule over an unbound List, like `sum_list(List, 0) :- :match_nil(List).`,
# fails analysis), then aggregate. Key each element by its position: facts are a
# set, so summing `:list:member(X, L)` over [2, 2, 5] gives 7, not 9.
suffix(ID, L) :- numbers(ID, L).
suffix(ID, Tail) :- suffix(ID, L), :match_cons(L, Head, Tail).
element(ID, Remaining, X) :- suffix(ID, L), :match_cons(L, X, Rest), Remaining = fn:list:len(L).
list_sum(ID, Total) :- element(ID, Remaining, X) |> do fn:group_by(ID), let Total = fn:sum(X).
```
...
fn:get(...)              # Use fn:list:get or fn:struct:get (fn:map:get is declared, never wired)
```
