...
# ========================================
Decl project(ID, Name, Version) bound [/name, /string, /string].
Decl depends_on(Project, Lib, Version) bound [/name, /name, /string].
Decl cve_affects(Lib, Version, CVE, Severity) bound [/name, /string, /string, /name].
Decl library_license(Lib, License) bound [/name, /string].
Decl acceptable_license(License) bound [/string].
Decl has_dependency(Project, Lib, Version) bound [/name, /name, /string].
Decl vulnerable_project(Project, CVE, Severity) bound [/name, /string, /name].
Decl license_violation(Project, Lib) bound [/name, /name].
Decl high_risk_project(Project) bound [/name].
Decl risk_summary(Project, VulnCount) bound [/name, /number].
...
# The library column is a name here too: /log4j and "log4j" never unify, and a join
# across the two spellings silently derives nothing.
...
library_license(/api_service, "Proprietary").
...
# === CHECK IT ===
# nerd check-mangle --standalone --eval vulnerable_project,license_violation,risk_summary sbom.mg
# derives vulnerable_project for /api_service and /data_processor (log4j, transitively)
# and /web_app (axios), and license_violation(/data_processor, /api_service).
```
...
# ========================================
Decl server(Name, Region, Env, Status) bound [/name, /name, /name, /name].
Decl contains_pii(Server, Flag) bound [/name, /name].
Decl approved_pii_region(Region) bound [/name].
Decl pii_violation(Server, Region) bound [/name, /name].
Decl availability_violation(Server) bound [/name].
Decl redundant(Server) bound [/name].
Decl single_point_of_failure(Server) bound [/name].
Decl compliance_issues(Type, Count) bound [/name, /number].
...
# Data classification (booleans are the names /true and /false; a bare `true` is a parse error)
...
# Compliance summary: the issue type is a constant in the head. (A trailing
# `|> let Type = /pii_violation.` does not bind, and the name swallows the period.)
compliance_issues(/pii_violation, Count) :-
    pii_violation(Server, Region) |>
    do fn:group_by(),
    let Count = fn:count().

compliance_issues(/availability_violation, Count) :-
    availability_violation(Server) |>
    do fn:group_by(),
    let Count = fn:count().
```
...
# ========================================
Decl volunteer(ID, Name, Skill) bound [/number, /string, /name].
Decl project(ID, Skill, Hours) bound [/name, /name, /number].
Decl available_hours(Volunteer, Hours) bound [/number, /number].
Decl can_help(Volunteer, Name, Project) bound [/number, /string, /name].
Decl can_commit(Volunteer, Project) bound [/number, /name].
Decl good_match(Volunteer, Name, Project) bound [/number, /string, /name].
Decl volunteers_per_project(Project, Count) bound [/name, /number].
Decl has_volunteers(Project) bound [/name].
Decl needs_volunteers(Project) bound [/name].
...
project(/translation, /languages, 6).
...
# Count volunteers per project (name every column in a transform rule's body)
...
# No volunteers at all: an aggregate over no rows derives nothing (not 0), so this
# is its own rule. Project first: `!volunteers_per_project(Project, _)` would be
# silently deleted by the engine and every project would "need volunteers".
has_volunteers(Project) :- volunteers_per_project(Project, _).
needs_volunteers(Project) :-
    project(Project, _, _),
    !has_volunteers(Project).

# Derives: needs_volunteers(/branding) (one volunteer), needs_volunteers(/translation) (none).
```
...
# ========================================
Decl direct_conn(From, To, Code, Price) bound [/name, /name, /string, /number].
Decl route(From, To, Codes, Price) bound [/name, /name, fn:List(/string), /number].
Decl cheapest_route(From, To, MinPrice) bound [/name, /name, /number].
...
# Find routes with up to one connection. Lists and values go in the head or in a
# body assignment; a `let` must apply a function (`let Price = P` is a parse error).
route(Start, Dest, [Code], Price) :-
    direct_conn(Start, Dest, Code, Price).

route(Start, Dest, [C1, C2], Price) :-
...
# Check it: nerd check-mangle --standalone --eval route,cheapest_route file.mg
```
...
Decl rare_condition(Y, Z).
Decl join_condition(X, Y).
Decl common_condition(X).
efficient(X, Y, Z) :- 
...
```mangle
Decl grant(User, Resource, Action).
Decl user_role(User, Role).
Decl role_permission(Role, Resource, Action).
Decl deny(User, Resource, Action).
Decl has_permission(User, Resource, Action).
Decl effective_permission(User, Resource, Action).
...
# Effective permissions (after denials): every variable of the negated atom is bound
...
### Temporal Reasoning
```mangle
Decl fact_validity(Fact, Start, End) bound [/name, /number, /number].
Decl query_time(Time) bound [/number].
Decl current_time(Now) bound [/number].
Decl valid_at(Fact, Time) bound [/name, /number].
Decl current_facts(Fact) bound [/name].

# Valid during a time period. Time must come from a relation: a variable that only
# appears in comparisons ranges over every number and fails analysis
# ("expected Time (arg 0) to be constant or bound variable").
valid_at(Fact, Time) :-
    query_time(Time),
...
### Provenance Tracking
```mangle
Decl base_fact(X, Y) bound [/name, /name].
Decl transformation(From, To) bound [/name, /name].
Decl derived(X, Y, Steps) bound [/name, /name, fn:List(/name)].

# Track the steps that derived each fact (the engine's DerivationRecorder is the
# real provenance mechanism; see 960-FORK_FEATURES)
derived(X, Y, [/base]) :- base_fact(X, Y).

derived(X, Z, Steps) :-
    derived(X, Y, Earlier),
    transformation(Y, Z),
    Steps = fn:list:cons(/transform, Earlier).
```
