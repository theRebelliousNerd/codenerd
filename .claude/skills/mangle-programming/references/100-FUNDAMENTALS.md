...
```mangle
# WRONG: negation through recursion. The engine reports
...
p(X) :- r(X), !q(X).
q(X) :- r(X), !p(X).
# What would it mean? No evaluation order is right: a paradox.
```
...
# Stratum 2 (uses stratum 1 negatively; negation is `!`, there is no `not`)
...
```
```mangle
# Mangle (comments are '#'; name every column in a transform rule's body)
...
Decl item(X, Value).
Decl person(X, Y, Z).
Decl employee(E, DeptID).
Decl department(DeptID, D).
Decl category1(X).
Decl category2(X).
Decl edge(X, Y).
...
Decl server(Name, Region, Env, Status).
Decl violation(Server).
violation(Server) :-
    server(Server, _, /production, Status),
    Status != /running .
# The space before the period matters: a name constant absorbs a following '.',
# so `Status != /running.` never ends the clause (see 020-ENGINE_TRUTHS).
```
