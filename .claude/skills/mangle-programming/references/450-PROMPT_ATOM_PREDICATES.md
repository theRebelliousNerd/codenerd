...
Decl prompt_atom(A1, A2, A3, A4, A5).
...
Decl prompt_atom(A1, A2, A3, A4, A5).
Decl atom_selector(X, Y, Z).
Decl compile_shard(X, Y).
Decl compile_context(X, Y).
Decl compile_query(Query).
Decl vector_recall_result(Query, AtomID, Similarity).
...
Decl atom_selected(AtomID).
Decl prompt_atom(A1, A2, A3, A4, A5).
Decl category_order(Category, CatOrder).
Decl atom_matches_context(AtomID, Score).
final_atom(AtomID, Order) :-
    atom_selected(AtomID),
...
Decl atom_candidate(AtomID).
Decl atom_conflict(AtomID, OtherID).
Decl atom_matches_context(AtomID, MyScore).
Decl atom_exclusion_group(AtomID, GroupID).
Decl prompt_atom(A1, A2, A3, A4, A5).
Decl atom_dependency_satisfied(AtomID).
atom_loses_conflict(AtomID) :-
...
Decl atom_selected(AtomID).
Decl compile_shard(X, Y).
Decl shard_executed(A1, A2, A3, A4).
...
These are the interactive interpreter's `?pred(...)` queries, not program text. The atom facts
exist only while a compile runs (the JIT kernel loads them for the compile and removes them
after), so `nerd query atom_selected` afterwards returns nothing: read
`.nerd/meter/atom-selections.jsonl`, which records what each compile selected.
...
Decl is_mandatory(Atom).
Decl blocked_by_context(Atom).
Decl vector_hit(Atom, Score).
