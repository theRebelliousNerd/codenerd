...
Decl sale(Region, Product, Amount, Quantity) bound [/name, /name, /float64, /number].
Decl employee(ID, Name, Department, Salary) bound [/name, /string, /name, /float64].
Decl project_assignment(EmployeeID, ProjectID, Hours) bound [/name, /name, /number].
Decl log_entry(Timestamp, Level, Message) bound [/number, /name, /string].
...
# Departments with at least 2 employees. A transform only holds do/let
# statements, so post-aggregation filtering is a second rule over the counts.
dept_size(Dept, Count) :-
    employee(_, _, Dept, _) |>
    do fn:group_by(Dept),
    let Count = fn:count().
large_department(Dept) :- dept_size(Dept, Count), Count >= 2.

# Projects with multiple assignees (same two-rule shape as above).
project_assignee_count(ProjectID, Count) :-
    project_assignment(_, ProjectID, _) |>
    do fn:group_by(ProjectID),
    let Count = fn:count().
shared_project(ProjectID) :- project_assignee_count(ProjectID, Count), Count > 1.
