...
# Users in the system
Decl user(ID, Name, Active) bound [/name, /string, /name].

# Roles in the system
Decl role(ID, Name, Level) bound [/name, /string, /number].

# Role hierarchy (parent role grants child role permissions)
Decl role_inherits(Child, Parent) bound [/name, /name].

# Direct role assignments
Decl user_role(User, Role) bound [/name, /name].

# Resources in the system
Decl resource(ID, Type, Owner) bound [/name, /name, /name].

# Permission grants (role can perform action on resource type)
Decl permission(Role, Action, ResourceType) bound [/name, /name, /name].

# Explicit denials (override grants)
Decl deny(User, Action, Resource) bound [/name, /name, /name].

# The actions a rule may range over. A head variable must be bound by a positive
# atom in the body, so "every action" needs a relation that lists them.
Decl action(Action) bound [/name].
action(/action/read).
action(/action/write).
...
resource(/res/scratch, /type/temp, /user/bob).   # no role has a /type/temp permission
...
permission_count(User, Count) :-
    has_permission_base(User, Action, Type) |>
    do fn:group_by(User),
    let Count = fn:count().

# Find resources with no access controls
# Project first: `!permission(_, _, Type)` would be silently deleted by the engine
# and every resource would be reported unprotected.
protected_type(Type) :- permission(_, _, Type).
...
# =============================================================================
# Queries (the ?pred form is the interactive interpreter's; from codeNERD:
#   nerd check-mangle --standalone --eval can_access,cannot_access,unprotected_resource access-control.mg)
# =============================================================================
