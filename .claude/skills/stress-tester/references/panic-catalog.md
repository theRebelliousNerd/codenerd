...
### P0: One-Shot CLI Hang After Result (process never exits)
...
### P0b: Workspace Write Pollution (`-w` ignored by tools)

**Location:** `internal/tools/core/workspace_guard.go` + boot path

**Trigger:** `nerd -w <other> create …` while process CWD is monorepo root.

**Symptom:** `write_file completed: C:\…\codeNERD\backend\…` instead of under `-w`; VirtualStore log still shows correct workingDir.

**Fix (2026-07):** `resolveWorkspaceRoot` sets `CODENERD_WORKSPACE_ROOT`.

**Stress Test:** see [workspace-isolation.md](workflows/09-cli-workspace-matrix/workspace-isolation.md).
