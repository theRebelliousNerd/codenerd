...
>
> **Delegation Hardening (Jun 2026):** Delegated tasks run with PRESET intents (`Executor.ProcessWithIntent`): the spawner/JITExecutor carry the routed `IntentVerb`, so subagents and inline runs never re-perceive the synthetic task string (no extra classification call, no persona roulette). Task intents are asserted under unique `/task_intent_N` IDs and retracted after the run — subagents never clobber the session's `/current_intent` on the shared kernel. Inline execution clones the executor (`CloneForTask`) instead of mutating the shared session executor. The API scheduler wakes waiters in PRIORITY order (interactive clients register `PriorityHigh`; `JITExecutor.ExecuteWithContext` propagates spawn priority via `CtxKeyPriority`). Articulation prompt sections are capped (context facts, findings, replayed user turns).
>
...
| **codenerd-codex-exec** | Codex Runtime | Codex exec skill injection, auth probes, schema validation, concurrency tuning |
