...
## SK-015: codenerd-codex-exec

**Name:** `codenerd-codex-exec`

**Domain:** Codex Runtime & Subscription-Backed Engine Operations
...
- Working on `codex exec` as the backend transport for `codenerd`
- Adding or debugging repo-skill injection for the Codex CLI backend
- Extending noninteractive auth/status checks for Codex subscription usage
- Fixing schema-constrained output handling for Codex CLI
- Tuning Codex-specific concurrency limits under subscription rate limits
- Auditing how JIT, Piggyback, and Codex skills interact

### Key Capabilities

| Capability | Description |
|------------|-------------|
| Backend Contract | Keeps Codex in the backend-model role while codeNERD remains the orchestrator |
| Repo Skill Injection | Documents and enforces explicit repo-local skill usage for `codex exec` |
| Auth Probing | Guides real noninteractive readiness probes instead of shallow install checks |
| Schema Discipline | Preserves schema-first behavior for Piggyback and structured health probes |
| Concurrency Tuning | Documents the conservative Codex-specific cap and rate-limit tuning path |
| Wiring Audit | Maps the real config/client/auth/scheduler touchpoints for Codex CLI integration |

### Bundled Resources

| Resource | Type | Purpose |
|----------|------|---------|
| `.agents/skills/codenerd-codex-exec/SKILL.md` | Core | Runtime contract for Codex-as-codeNERD-backend behavior |
| `.agents/skills/codenerd-codex-exec/agents/openai.yaml` | Metadata | Marks the skill as explicit-only |
| `.agents/skills/codenerd-codex-exec/scripts/codex_exec_probe.py` | Script | Deterministic auth, skill, schema, and bounded concurrency probes |
| `.agents/skills/codenerd-codex-exec/references/wiring-map.md` | Reference | Real engine/config/auth/scheduler wiring map |
| `.agents/skills/codenerd-codex-exec/references/auth-and-probes.md` | Reference | Auth, login, and health-probe guidance |
| `.agents/skills/codenerd-codex-exec/references/concurrency-and-rate-limits.md` | Reference | Codex-specific concurrency and rate-limit tuning |
| `.agents/skills/codenerd-codex-exec/references/schema-jit-piggyback.md` | Reference | JIT/Piggyback/schema interaction rules |

### Integration Points

| Integrates With | Relationship |
|-----------------|--------------|
| cli-engine-integration | Specializes the Codex CLI backend for repo-local `codex exec` usage |
...
| 2026-03-06 | Added SK-015 codenerd-codex-exec | Codex |
