...
Gemini 2.5 models do **not require** thought signatures for function calling. Signatures are optional and will be ignored if provided. This allows the same code to work with both model families:

```go
func (c *GeminiClient) requiresThoughtSignature() bool {
    return strings.Contains(c.model, "gemini-3")
}
```
