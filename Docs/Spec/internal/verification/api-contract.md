# API Contract: internal/verification

## Introduction
This document defines the application programming interface boundaries for the `internal/verification` subsystem. It covers the exported types and functions that other components interact with to perform quality-enforcing loop checks on LLM outputs.

## Exported Types

### TaskVerifier
The primary struct exposed by this package is `TaskVerifier`.
...
### VerificationResult
Returned by the verification functions to summarize LLM task validation.
...
### VerifyWithRetry
`func (v *TaskVerifier) VerifyWithRetry(ctx context.Context, task string, shardType string, maxRetries int) (string, *VerificationResult, error)`
This is the main entry point. It loops until success or maximum retries are reached, executing the task, verifying quality via LLM, and applying corrective actions.

### SetTaskExecutor
`func (v *TaskVerifier) SetTaskExecutor(te session.TaskExecutor)`
Configures the unified task executor for shard execution.

### SetSessionContext
`func (v *TaskVerifier) SetSessionContext(sessionID string, turnCount int)`
Injects runtime session details used for persistence and metrics tracking.
