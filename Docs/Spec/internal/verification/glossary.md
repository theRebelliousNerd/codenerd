# Glossary: internal/verification

## Core Verification Terms

| Term | Definition |
| :--- | :--- |
| `TaskVerifier` | The core struct managing the LLM evaluation and retry loop execution. |
| `VerificationResult` | Stores the LLM decision metrics such as success status, confidence score, and specific violation records. |
| `QualityViolation` | Typed string enumeration indicating specific code flaws (e.g., `MockCode`). |
| `CorrectiveAction` | An object dictating how the system should handle a rejection (e.g., retrieving additional docs or asking a specialist). |
| `ErrMaxRetriesExceeded` | Emitted when the iteration loop surpasses `maxRetries` (default 3) without achieving a clean verification pass. |
| `ShardSelectionResult` | Tracks the heuristic or LLM-derived optimal shard to handle the task retry. |
