# Configuration: internal/verification

## Overview
The `internal/verification` subsystem relies on programmatic dependency injection rather than file-based or environment variables. All state is passed in via constructor patterns.

## Injection Points

The `NewTaskVerifier` function requires the following injected dependencies:
- `perception.LLMClient`
- `store.LocalStore`
- `coreshards.ShardManager`
- `autopoiesis.Orchestrator`

## Retries Configuration
The `maxRetries` parameter in `VerifyWithRetry` acts as a primary tuning knob for the verification loop. If a value less than or equal to zero is provided, the system defaults to 3 retries.

## LLM Prompts
The subsystem embeds string constants for exact LLM prompt instructions:
- Implementation Task Prompt: Enforces strict checks against `MockCode`, `PlaceholderCode`, `HallucinatedAPI`, and `FakeTests`.
- Review Task Prompt: Validates that analysis output is useful without failing on the analyzed code's own incomplete state.

## Limits and Truncation
There are hardcoded configuration limits for string truncation during LLM verification to prevent context overflow.
- `truncateForVerification`: Limits responses to 8000 characters.
- `truncateContext`: Customizable limit per call, frequently configured to 2000 characters when appending specialist knowledge.
