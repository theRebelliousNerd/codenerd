# Observability: internal/verification

## Internal Telemetry
The subsystem generates no structured logs for successful task passes outside of standard session telemetry handled higher in the stack. 

However, critical failure paths are surfaced continuously via the `internal/logging` package:
- `logging.SystemShardsWarn` catches JSON marshaling errors while storing metrics for `QualityViolations`, `Evidence`, and `CorrectiveAction`.
- `logging.StoreError` fires when the `localDB.StoreVerification` call fails preventing the system from appending data for the continuous learning feedback loops.

## Database Metrics
`TaskVerifier` persists the outcome of every LLM assertion. The hashed `taskHashHex` permits downstream systems and the autopoiesis framework to analyze historical `confidence` ratings overlaid against the `shardType` attempts, identifying patterns of serial model weakness.
