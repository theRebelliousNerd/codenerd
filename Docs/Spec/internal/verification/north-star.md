# North Star: internal/verification

## Vision Statement
The `internal/verification` subsystem acts as the codeNERD quality gate. Its primary directive is to strictly prohibit the injection of "lazy LLM code" into the primary working directory or the `VirtualStore`. Tasks are not done until they are proven robust, executable, and fully wired.

## Goal Structure
Any generated output must be scrubbed for mock code, magic strings, ignored errors, or pseudo-implementations holding `TODO` and `panic("not implemented")`. The ultimate objective is an asynchronous loop that actively corrects generating models by shifting execution context to appropriate specialist shards automatically prior to surfacing errors to the developer.

## Design Philosophy
We exchange execution speed for guaranteed structural integrity. Executing tasks internally until they pass deterministic and LLM-assessed checks removes the burden of review from the user.
...
## Non-Goals
Do not replace main LLM.


## Uplift Roadmap
Phase 1: Implement verifier.go
