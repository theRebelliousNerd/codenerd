...
```textUser Input → LLM (Parse to Candidates) → Mangle Validator → [Pass/Fail]
                    ↑                                           ↓
...
**Data flow architecture:**
```textSensor/Input → LLM (Perception) → Probabilistic Facts → Mangle (Cognition) → Facts → LLM (Articulation) → Output
                     ↓                                         ↓
...
**Example application:**
```textQuery: can_execute(user_123, delete_file, doc_456)
Result: FAILED
...
```textAgent observes: When user says "urgent," priority should be high
...
```text% Meta-predicates for control flow
...
```text_block_response :- response_contains(harmful_content).
...
```textaction_permitted(User, Action) :- explicitly_permitted(User, Action).
...
```text% Only compute top-10 results
...
```textcontradiction_detected :- fact(P), fact(not(P)).
