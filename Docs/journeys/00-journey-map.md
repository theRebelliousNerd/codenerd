...
| 0.5 | ~~Greeting fast path~~ | deleted 2026-09-23 | a greeting goes through perception and takes the kernel's `/perception_answer` lane (row 1c) | — | — | — | — | — |
...
| 1c | Perception answer | `process.go` after the kernel seed (row 2) | end the turn on perception's own `surface_response`, skip the rest of ORIENT/DECIDE/ACT | **kernel** since 2026-09-23: `route_decision(/perception_answer)` (`routing_arbitration.mg`: `perception_answer_verb`/`perception_answer_target`, `intent_signal(/has_surface_response)`, no shard). Was Go (`willConverse`, `isConversationalIntent`), plus a duplicate late in the flow | none beyond perception | n/a | none | no verdict |
...
| 5 | Dream | `process.go` (`RouteDream`) | consult the shards on a hypothetical → `handleDreamState` | **kernel** since 2026-09-23: `route_decision(/dream)`; a /dream turn perception already answered takes row 1c | — | — | — | see Journey 5 |
...
| 7 | **Routing arbitration (the one real DECIDE point)** | `process.go` `m.decideRoute` → `delegation_routing.go` | one of `perception_answer / dream / respond_directly / multi_step / clarify / delegate`, or none | **kernel** — `route_decision/2` in `policy/routing_arbitration.mg`, which derives one lane at most and carries the precedence itself; no lane = articulation answers. Since 2026-09-23 a nil kernel, a query error or an empty derivation decides nothing (`RouteNone`); `RouteLegacy` and the Go precedence switch are deleted. | EDB asserted per turn (retract first): `delegation_candidate`, `multi_step_signal`, `multi_step_plan_step`, `intent_signal`, `config_param(/routing_...)` | — | — | — |
| 8 | Clarify | `process.go` (`RouteClarify`) | ask before acting: the kernel's own question (`kernelClarification`), else the clarifier shard, else the fallback question | **kernel** since 2026-09-23: `clarify_lane()` over `needs_clarification()` -- an uncertain mutation, an untargeted request whose verb acts on a target, or a derived `clarification_question`; delegate yields to it. The Go heuristics `shouldAutoClarify` (keyword match) and `shouldClarifyIntent` are deleted | clarifier shard output | — | clarifier shard | returns prose + `ClarifyUpdate`, no verdict |
| 9 | Multi-step | `process.go` (`RouteMultiStep`) | run the plan the kernel judged (`route.Steps`) | **kernel** since 2026-09-23: `multi_step_lane()` needs `is_multi_step()` (signals, incl. the corpus's clause patterns as `/corpus_pattern`) **and** `multi_step_plan_ready()` -- `decomposeTask`'s plan, asserted as `multi_step_plan_step(Index, Shard)` before the ask, has two steps or more and each names a shard. Was: the lane derived on signals alone and Go re-decomposed after it; a one-step plan fell through, neither decomposed nor delegated. | `decomposeTask` in `decideRoute` | the step list is the obligation | — | per-step |
...
- **Clarify.** (2026-09-23: one path now, the kernel's `/clarify` lane; the text below
  is the 2026-09-18 measurement.) Three paths again: `shouldAutoClarify` (Go), `shouldClarifyFromKernel`
