...
| 2026-09-21 15:19 | R6 | nebulous brief, internal/features, with the grader as the campaign's acceptance witness (R6-F1) | campaign `--type feature --docs --accept` | **stopped by hand at 18 of 31 tasks** when Steve needed the GPU: phases 1-4 whole, grader 16 -> 7, the 7 being exactly the phase 5-6 slots not yet reached. First reading of the cache-order change: 68% of input cached (49% before). Forced six hand fixes (dogfood ledger, 2026-09-21 evening): a repeat after a write failed the task and the rollback deleted the write (`03-GAP-ANALYSIS.md` three times); `callers_of` blind to package-level initializers, which made the spec call a live function test-only -- the campaign's own review caught it; rolling-wave scope refresh denied on every campaign ever; one file deleted mid-walk aborted the workspace scan; four predicates the kernel could not see | 135 | -- | 8 new, 1 changed | not committed: resume |

## Program of work from Steve, 2026-09-21 (evening)

Asked for in one sitting, recorded so it outlives the session. Status is as of `90e43566` plus the
embedding change that follows it. Hand-built items say so; the rest are candidates for codeNERD.

| # | Asked for | Status |
|---|---|---|
| 1 | A system that checks `config.json` and raises errors | **done** (`90e43566`): `UserConfig.Check`, loader refuses contradictions, `main()` exits 2, `nerd config check` |
| 2 | Route to multiple providers and models | **done** (`90e43566`): `shard_profiles.<x>.provider`, applied on every path that runs a persona |
| 3 | Every configurable field visible in `config.json`, set by hand, nothing defaulted silently | **partial**: `nerd config full` writes the whole document and `check` lists every implicit field (34 in the live file); defaults still exist in code |
| 4 | Everything on Muse Spark 1.3 contributor, max settings | **done**: five profiles, 1M `context_window.max_tokens`; `xhigh` and 131k output were already set. The real window is unverified |
| 5 | Everything enabled: learning, yolo, tools, max permissiveness | **done**: `yolo`, `continuation_mode` 0, `tool_generation.allow_tool_exec`, `features.prompt_evolution`, `learning_candidate_auto_promote`, reviewer `enable_learning`; `taxonomy_fast` off (it only makes `verify_taxonomy` skip its sweep). `per_shard_facts` stays off: no cross-shard join coordinator exists, so it silently deletes cross-domain derivations (6 split joins, 10 blind negations in the audit's residue) -- building the coordinator is the work |
| 6 | A better local embedding model (embeddinggemma's 2k context broke recall) | **done**: `qwen3-embedding:4b` (40,960 context, 2,560 dims, GPU); `embedding.dimensions` replaces a hardcoded 768 and is verified against the first vector; one `EmbeddingConfig.EngineConfig()` replaces twelve hand-copies, two of which never read the user's config; a context-length error is no longer retried |
| 7 | `max_retries` is an arbitrary 2 or 3 | open: a hardcoded init default; derive it or replace it with a progress stop (the hardcoded-limits class) |
| 8 | Expert spawning much more sophisticated; forced when the agent does not choose to | open, **diagnosed**: (a) the Go matcher `MatchSpecialistsForTask` (a hardcoded `CoreTechnologyPatterns` table) has two callers, both in `cmd/nerd/chat` -- on `nerd fix` and campaigns no expert is ever selected; (b) the policy's `delegate_task(Specialist, ...)` (`shards.mg:155`) joins `shard_performing_well` and `shard_can_handle`, which nothing asserts, so it cannot fire. Build: the kernel derives `expert_needed(Task, Agent, Reason)` from asserted facts (agent domains from `agents.json` and KB, the technologies of the task's files from the world model, the verb); the executor and campaign paths consume it; the expert's knowledge is pushed into the window by JIT first, a spawn second -- the harness decides, not the model's willingness |
| 9 | The subagent system better overall | open |
| 10 | Provider feature toggles in config; Meta search grounding for every agent on Spark | open: only `gemini` has a feature block; Meta grounding exists as a separate call (`client_openai_compat_grounding.go`) and is not offered inside the Spark tool loop |
| 11 | `/init` infers the codebase's north star from deep study | open |
| 12 | Init builds experts for the codebase's domains holistically, not only library experts | open |
| 13 | Dependencies update automatically with compatibility guarantees; experts prune old expertise and grow new when packages change | open |
| 14 | Every field of every expert KB and embedded database is filled, guaranteed | open: a gate in the starved-predicate style is the first piece |
| 15 | Run with everything on, and use codeNERD's own architecture review / enhancement and pruning while dogfooding | open; the `.nerd/context/` half is **done** (lane B wave 4): every archive carries an owner record (pid, host), `internal/context/working_retention.mg` derives `working_archive_prunable` for an archive whose owner process is gone or retired its scope (never by age), a task clone or subagent retires its scope when its task returns, session boot prunes, `nerd memory prune [--dry-run]` prunes on demand, and `nerd status` / `nerd memory status` print the counts. Proven by `TestJITExecutor_AnInlineTaskLeavesNoWorkingArchive`, `TestWorkingRetention_KeepsWhatCanBeRedeemedAndPrunesTheRest`, `TestWorkspaceState_StatusCountsArchivesAndPruneRemovesTheDeadOnes`. Resuming a scope across runs is still N13 |
| 16 | Muse sampling: does it take temperature and top_p | answered: the Meta branch sends temperature, and top_p only when no temperature is set (the API rejects both); every profile sets both, so top_p is dropped |
