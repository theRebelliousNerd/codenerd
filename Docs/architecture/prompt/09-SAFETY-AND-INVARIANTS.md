...
### I3 — compile_context cleanup

When kernel implements `KernelRetracter`, Compile defers `Retract("compile_context")` after successful assert. Prevents cross-turn selection pollution.
...
### I8 — Singleflight cache identity

Concurrent identical context hashes share one compilation; cache returns same result pointer semantics — callers must treat results as read-only.
...
`Generate` fails if no ConfigAtom found (except consult→general fallback). Empty tool lists are possible only if atom defines empty tools — callers must not invent tools at runtime outside config.
...
| Leaving compile_context unretracted | Poisoned selection |
