...
| Area | Status | Notes |
|------|--------|-------|
| Package on disk | Yes | `internal/regression/` |
| Source files | 1 | non-test .go |
| Tests | 1 | `*_test.go` |
| Types sampled | 3 | export scan |
...
3. Test gaps if test count << source count (currently 1 vs 1).
4. Docs/Spec 18-file product templates remain a separate `spec-doc-sprint` track.

## Non-gaps

Implementation exists under `internal/regression/`; do not treat as pre-implementation 0%.
