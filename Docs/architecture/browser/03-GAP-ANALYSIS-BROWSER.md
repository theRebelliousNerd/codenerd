...
| Area | Status | Notes |
|------|--------|-------|
| Package on disk | Yes | `internal/browser/` |
| Source files | 3 | non-test .go |
| Tests | 6 | `*_test.go` |
...
3. Test gaps if test count << source count (currently 6 vs 3).
4. Docs/Spec 18-file product templates remain a separate `spec-doc-sprint` track.

## Non-gaps

Implementation exists under `internal/browser/`; do not treat as pre-implementation 0%.
