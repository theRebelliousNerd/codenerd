...
| **Incremental / differential eval** | **3** | `DifferentialEngine` + unified fast path exist; kernel routes via `features.IsDiffEvalEnabled()`. Gaps: no external-predicate / gas-option forwarding on legacy path; invalidates on retract/policy; operational guidance often prefers full path. |
