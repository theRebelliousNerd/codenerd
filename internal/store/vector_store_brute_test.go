...
func TestVectorStore_FilterUtils(t *testing.T) {
	...
	expected := "SELECT id, content, embedding, metadata, created_at FROM vectors WHERE embedding IS NOT NULL AND (json_extract(metadata, '$.path') = ? OR json_extract(metadata, '$.path') = ?)"
