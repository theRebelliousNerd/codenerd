...
var (
	...
	entryPatternRegexp  = regexp.MustCompile(`(\[TOOL_ENTRY\][^\n]+\n)`)
	errorReturnPattern  = regexp.MustCompile(`return\s+([^,\n]+),\s*(fmt\.Errorf|errors\.New|err)\b`)
	httpPatternRegexp   = regexp.MustCompile(`(http\.(Get|Post|Do)\([^)]+\))`)
	forPatternRegexp    = regexp.MustCompile(`(for\s+[^{]+\{)\n`)
	errorLogPattern     = regexp.MustCompile(`if\s+err\s*!=\s*nil.*log\.`)
	stepPatternRegexp   = regexp.MustCompile(`(?m)^(?:\d+\.|[-*])\s*(.+)$`)
...
func (li *LogInjector) injectExitLogging(code string, toolName string) string {
	...
	return entryPatternRegexp.ReplaceAllStringFunc(code, func(match string) string {
...
func (li *LogInjector) injectErrorLogging(code string, toolName string) string {
	...
	return errorReturnPattern.ReplaceAllStringFunc(code, func(match string) string {
		...
		parts := errorReturnPattern.FindStringSubmatch(match)
		if len(parts) < 3 {
			return match
		}
...
func (li *LogInjector) injectAPICallLogging(code string, toolName string) string {
	...
	return httpPatternRegexp.ReplaceAllStringFunc(code, func(match string) string {
...
func (li *LogInjector) injectIterationLogging(code string, toolName string) string {
	...
	counter := 0
	return forPatternRegexp.ReplaceAllStringFunc(code, func(match string) string {
		counter++
...
func hasErrorLogging(code string) bool {
	return strings.Contains(code, "TOOL_ERROR") || errorLogPattern.MatchString(code)
}
...
func extractThoughtSteps(response string) []ThoughtStep {
	...
	matches := stepPatternRegexp.FindAllStringSubmatch(response, -1)
