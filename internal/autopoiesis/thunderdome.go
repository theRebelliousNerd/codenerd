	...
	ast.Inspect(file, func(n ast.Node) bool {
		...
		if fn.Type.Results != nil {
			...
			if len(results) == 2 {
				...
				score += 3
				if t.isErrorResult(results[0]) {
					callExpr = strings.Replace(callExpr, "_, toolErr =", "toolErr =", 1)
				} else {
					callExpr = strings.Replace(callExpr, "_, toolErr =", "_ =", 1)
				}
			} else if len(results) > 2 {
				blanks := strings.Repeat("_, ", len(results)-1) + "_"
				callExpr = strings.Replace(callExpr, "_, toolErr =", blanks+" =", 1)
			}
		} else {
			// 0 return values
			callExpr = strings.Replace(callExpr, "_, toolErr = ", "", 1)
		}
