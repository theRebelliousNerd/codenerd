...
// TODO: TEST_GAP: [Null/Undefined/Empty] Verify tool execution properly handles and identifies tools that panic when an empty string ("") is supplied as the attack input.
// TODO: TEST_GAP: [Type Coercion] Verify what happens if the input attack vector is malformed or has incorrect fields in thunderdome_harness_test.go Battle.
...
// TODO: TEST_GAP: [User Request Extremes] Verify runAttack does not OOM the host when a tool outputs massive string data to stdout/stderr. Enforce a LimitReader on the exec.Cmd output pipes.
...
// TODO: TEST_GAP: [User Request Extremes] Verify prepareArena aborts and returns an error if go test -c takes longer than a strict internal timeout, preventing infinite compilation attacks.
...
// TODO: TEST_GAP: [State Conflicts] Verify Battle handles tools that catch their own panics (using recover()) internally instead of returning an error.
// TODO: TEST_GAP: [State Conflicts] Verify Battle detects panics resulting from deep recursion (Go runtime stack overflow) properly instead of marking it a survival/timeout.
