// Package tool_generator implements the ToolGenerator ShardAgent for autopoiesis operations.
...
func (s *ToolGeneratorShard) SetLLMClient(client core.LLMClient) {
	...
	if s.orchestrator == nil {
		...
		if s.generatorConfig.ToolsDir != "" {
			if filepath.IsAbs(s.generatorConfig.ToolsDir) {
				autoConfig.ToolsDir = s.generatorConfig.ToolsDir
			} else {
				autoConfig.ToolsDir = filepath.Join(workspace, s.generatorConfig.ToolsDir)
			}
		}
		autoConfig.AgentsDir = filepath.Join(workspace, ".nerd", "agents")
