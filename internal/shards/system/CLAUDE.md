...
| `legislator.go` | LegislatorShard translating corrective feedback into durable Mangle policy rules. Exports llmClientAdapter wrapping types.LLMClient for FeedbackLoop, Piggyback Protocol processing, and hot-loading to `learned.mg` (kernel policy rules). Taxonomy learning is separate and persists to `.nerd/mangle/learned_taxonomy.mg` via the perception taxonomy engine. **NOTE (Dec 2024):** The hardcoded `legislatorSystemPrompt` was deleted; Legislator now uses JIT-compiled prompts from `internal/prompt/atoms/identity/legislator.yaml` which contains CORRECT declared predicates. |
