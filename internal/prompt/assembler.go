...
// assembleWithConfig runs assembly against an explicit snapshot of the
// assembler's config, taken under the read lock, so a concurrent setter cannot
// change the configuration halfway through one assembly.
