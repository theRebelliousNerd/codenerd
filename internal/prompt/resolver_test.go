...
func TestDependencyResolver_Resolve(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "malformed atom IDs with unicode, spaces and control chars",
			atoms: []*ScoredAtom{
				{Atom: &PromptAtom{ID: "atom 1", DependsOn: []string{"atom 2"}}, Combined: 0.5},
				{Atom: &PromptAtom{ID: "atom 2"}, Combined: 0.6},
				{Atom: &PromptAtom{ID: "atom_🔥"}, Combined: 0.7},
				{Atom: &PromptAtom{ID: "atom\t\n3", DependsOn: []string{"atom_🔥"}}, Combined: 0.8},
			},
			expectError:   false,
			expectedLen:   4,
			expectedOrder: []string{"atom_🔥", "atom\t\n3", "atom 2", "atom 1"},
		},
