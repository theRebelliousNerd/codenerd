...
func DetectCodePatterns(content string, elements []CodeElement) CodePatterns {
	...
	// Check for generated code markers in a deterministic order (longest/most specific first)
	type markerInfo struct {
		marker string
		gen    string
	}
	generatedMarkers := []markerInfo{
	...
	for _, info := range generatedMarkers {
		if strings.Contains(content, info.marker) {
			patterns.IsGenerated = true
			patterns.Generator = info.gen
			patterns.GeneratorMarker = info.marker
			break
		}
	}
