...
mapped(M) :- numbers(N), M = fn:map(N, fn:double(2)).   # ERROR: No map function (and no fn:double)
reduced(R) :- numbers(N), R = fn:reduce(N, fn:plus(1, 2)).  # ERROR: No reduce function

# Test 3: SQL-style date functions
Decl timestamp(T) bound [/string].
...
Decl number_string(S) bound [/string].
number_string("42").
...
write_file(R) :- R = fn:write_file("/path", "data").            # ERROR: No IO (and unknown fn)
...
Decl x(X) bound [/number].
Decl y(Y) bound [/number].
x(10).
y(5).
# CORRECT: These are real Mangle functions
sum(S) :- x(X), y(Y), S = fn:plus(X, Y).
product(P) :- x(X), y(Y), P = fn:mult(X, Y).
