...
description: Master prompt engineering for codeNERD's neuro-symbolic architecture. This skill should be used when the user asks to write or audit shard prompts, create or edit prompt atoms, debug LLM behavior, tune context injection or JIT prompt assembly, or implement Piggyback protocol and tool steering.
---
...
|----------|---------|------|
| **Writing New Shards** | When creating or updating shard prompts, prompt atoms, or shard generation templates. | Define the shard's persona, capabilities, and interface with the kernel. |
| **Auditing Prompts** | When reviewing prompt changes, Piggyback compliance, or tool steering (including `audit_prompts.py`). | Ensure strict adherence to the Piggyback Protocol and Constitutional boundaries. |
| **Debugging LLM Behavior** | When the model ignores tools, violates Piggyback ordering, or breaks control-packet rules. | Identify "Context Starvation" or "Ambiguous Steering" in the prompt chain. |
| **Optimizing Context** | When tuning JIT prompt assembly, context injection, `SessionContext`, or Spreading Activation. | Maximize relevance per token; strictly curate what enters the "Working Memory". |
| **Creating Specialists** | When defining Type B/U persistent specialists or hydration prompts. | Inject domain-specific "Knowledge Atoms" to hydrate the specialist's expertise. |
