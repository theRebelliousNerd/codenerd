...
func runTestContext(cmd *cobra.Command, args []string) error {
	...
	// Type assert to get *core.RealKernel from the interface. Bail out
	// loudly rather than passing a nil kernel into the harness: the
	// RealIntegrationEngine branch below dereferences this pointer for
	// every fact-seeder call, and the previous silent fallback masked
	// the underlying configuration error with a downstream nil-panic.
	realKernel, ok := cortex.Kernel.(*core.RealKernel)
	if !ok || realKernel == nil {
		return fmt.Errorf("test-context: cortex.Kernel is %T, expected *core.RealKernel", cortex.Kernel)
	}
